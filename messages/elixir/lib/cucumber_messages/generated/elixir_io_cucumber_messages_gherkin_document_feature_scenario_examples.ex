...
defmodule(Io.Cucumber.Messages.GherkinDocument.Feature.Scenario.Examples) do
  @moduledoc false
  (
    defstruct(
      location: nil,
    ...
    (
      ...
      (
        ...
        def(decode!(bytes)) do
          parse_key_value(
            bytes,
            struct(Io.Cucumber.Messages.GherkinDocument.Feature.Scenario.Examples)
          )
        end
      )
          ...
          {field, rest} =
            case(Protox.Decode.parse_key(bytes)) do
              ...
              {6, _, bytes} ->
                {len, bytes} = Protox.Varint.decode(bytes)
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = Io.Cucumber.Messages.GherkinDocument.Feature.TableRow.decode!(delimited)
    ...
    def(defs_by_name()) do
      %{
        ...
        table_body:
          {7, :unpacked, {:message, Io.Cucumber.Messages.GherkinDocument.Feature.TableRow}},
        table_header:
          {6, {:default, nil}, {:message, Io.Cucumber.Messages.GherkinDocument.Feature.TableRow}},
