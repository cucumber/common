...
defmodule(Io.Cucumber.Messages.TestStepFinished) do
  @moduledoc false
  (
    defstruct(
    ...
    (
      ...
      (
        ...
        def(decode!(bytes)) do
          parse_key_value(bytes, struct(Io.Cucumber.Messages.TestStepFinished))
        end
      )
          ...
          {field, rest} =
            case(Protox.Decode.parse_key(bytes)) do
              ...
              {1, _, bytes} ->
                ...
                value = Io.Cucumber.Messages.TestStepFinished.TestStepResult.decode!(delimited)
              ...
              {2, _, bytes} ->
                {len, bytes} = Protox.Varint.decode(bytes)
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = Io.Cucumber.Messages.Timestamp.decode!(delimited)
      ...
      %{
        1 =>
          {:test_step_result, {:default, nil},
           {:message, Io.Cucumber.Messages.TestStepFinished.TestStepResult}},
        2 => {:timestamp, {:default, nil}, {:message, Io.Cucumber.Messages.Timestamp}},
    ...
    def(defs_by_name()) do
      %{
        ...
        test_step_result:
          {1, {:default, nil}, {:message, Io.Cucumber.Messages.TestStepFinished.TestStepResult}},
        timestamp: {2, {:default, nil}, {:message, Io.Cucumber.Messages.Timestamp}}
      }
    end
