...
defmodule(Io.Cucumber.Messages.TestCase) do
  @moduledoc false
  (
    ...
    (
      ...
      (
        ...
        def(decode!(bytes)) do
          parse_key_value(bytes, struct(Io.Cucumber.Messages.TestCase))
        end
      )
          ...
          {field, rest} =
            case(Protox.Decode.parse_key(bytes)) do
              ...
              {3, _, bytes} ->
                {len, bytes} = Protox.Varint.decode(bytes)
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = Io.Cucumber.Messages.TestCase.TestStep.decode!(delimited)
    ...
    def(defs()) do
      %{
        ...
        3 => {:test_steps, :unpacked, {:message, Io.Cucumber.Messages.TestCase.TestStep}}
      }
    end
    ...
    def(defs_by_name()) do
      %{
        ...
        test_steps: {3, :unpacked, {:message, Io.Cucumber.Messages.TestCase.TestStep}}
      }
    end
