...
defmodule(Io.Cucumber.Messages.GherkinDocument.Feature.Scenario) do
  @moduledoc false
  (
    defstruct(
      location: nil,
    ...
    (
      ...
      (
        ...
        def(decode!(bytes)) do
          parse_key_value(bytes, struct(Io.Cucumber.Messages.GherkinDocument.Feature.Scenario))
        end
      )
          ...
          {field, rest} =
            case(Protox.Decode.parse_key(bytes)) do
              ...
              {2, _, bytes} ->
                {len, bytes} = Protox.Varint.decode(bytes)
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = Io.Cucumber.Messages.GherkinDocument.Feature.Tag.decode!(delimited)
              ...
              {6, _, bytes} ->
                {len, bytes} = Protox.Varint.decode(bytes)
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = Io.Cucumber.Messages.GherkinDocument.Feature.Step.decode!(delimited)
              ...
              {7, _, bytes} ->
                ...
                value =
                  Io.Cucumber.Messages.GherkinDocument.Feature.Scenario.Examples.decode!(
                    delimited
                  )
    ...
    def(defs()) do
      %{
        ...
        2 => {:tags, :unpacked, {:message, Io.Cucumber.Messages.GherkinDocument.Feature.Tag}},
        3 => {:keyword, {:default, ""}, :string},
        4 => {:name, {:default, ""}, :string},
        5 => {:description, {:default, ""}, :string},
        6 => {:steps, :unpacked, {:message, Io.Cucumber.Messages.GherkinDocument.Feature.Step}},
        7 =>
          {:examples, :unpacked,
    ...
    def(defs_by_name()) do
      %{
        ...
        steps: {6, :unpacked, {:message, Io.Cucumber.Messages.GherkinDocument.Feature.Step}},
        tags: {2, :unpacked, {:message, Io.Cucumber.Messages.GherkinDocument.Feature.Tag}}
      }
    end
