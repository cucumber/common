...
defmodule(CucumberMessages.Io.Cucumber.Messages.SourceReference.JavaMethod) do
  @moduledoc false
  (
    ...
    (
      ...
      (
        ...
        def(decode!(bytes)) do
          parse_key_value(
            bytes,
            struct(CucumberMessages.Io.Cucumber.Messages.SourceReference.JavaMethod)
          )
        end
      )
          ...
          {field, rest} =
            case(Protox.Decode.parse_key(bytes)) do
              ...
              {3, _, bytes} ->
                ...
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = delimited
                field = {:method_parameter_types, msg.method_parameter_types ++ List.wrap(value)}
                {[field], rest}
