...
defmodule(Io.Cucumber.Messages.GherkinDocument.Feature.FeatureChild) do
  @moduledoc false
  (
    ...
    (
      ...
      (
        ...
        def(decode!(bytes)) do
          parse_key_value(
            bytes,
            struct(Io.Cucumber.Messages.GherkinDocument.Feature.FeatureChild)
          )
        end
      )
          ...
          {field, rest} =
            case(Protox.Decode.parse_key(bytes)) do
              ...
              {1, _, bytes} ->
                ...
                value =
                  Io.Cucumber.Messages.GherkinDocument.Feature.FeatureChild.Rule.decode!(
                    delimited
                  )
              ...
              {2, _, bytes} ->
                {len, bytes} = Protox.Varint.decode(bytes)
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = Io.Cucumber.Messages.GherkinDocument.Feature.Background.decode!(delimited)
              ...
              {3, _, bytes} ->
                {len, bytes} = Protox.Varint.decode(bytes)
                <<delimited::binary-size(len), rest::binary>> = bytes
                value = Io.Cucumber.Messages.GherkinDocument.Feature.Scenario.decode!(delimited)
      ...
      %{
        ...
        rule:
          {1, {:oneof, :value},
           {:message, Io.Cucumber.Messages.GherkinDocument.Feature.FeatureChild.Rule}},
        scenario:
          {3, {:oneof, :value}, {:message, Io.Cucumber.Messages.GherkinDocument.Feature.Scenario}}
      }
    end
