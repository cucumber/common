...
var tree_regexp_1 = __importDefault(require("../src/tree_regexp"));
describe("TreeRegexp", function () {
    it("exposes group source", function () {
        var tr = new tree_regexp_1.default(/(a(?:b)?)(c)/);
    ...
    it("matches optional group", function () {
        var tr = new tree_regexp_1.default(/^Something( with an optional argument)?/);
        var group = tr.match("Something");
        assert_1.default.strictEqual(group.children[0].value, null);
    });
    it("matches nested groups", function () {
        var tr = new tree_regexp_1.default(/^A (\d+) thick line from ((\d+),\s*(\d+),\s*(\d+)) to ((\d+),\s*(\d+),\s*(\d+))/);
    ...
    it("captures non capturing groups with capturing groups inside", function () {
        var tr = new tree_regexp_1.default('the stdout(?: from "(.*?)")?');
    ...
    it("works with flags", function () {
        var tr = new tree_regexp_1.default(/HELLO/i);
    ...
    it("does not consider parenthesis in character class as group", function () {
        var tr = new tree_regexp_1.default(/^drawings: ([A-Z, ()]+)$/);
...
//# sourceMappingURL=tree_regexp_test.js.map